...
print(f"[streamlit_app] Platform: {sys.platform}")
import struct
print(f"[streamlit_app] Pointer size (bits): {struct.calcsize('P') * 8}")
print(f"[streamlit_app] Default int dtype: {np.array([1]).dtype}")
print(f"[streamlit_app] Default float dtype: {np.array([1.0]).dtype}")
print(f"[streamlit_app] Float64 epsilon: {np.finfo(np.float64).eps}")
print(f"[streamlit_app] Int32 max: {np.iinfo(np.int32).max}")
print(f"[streamlit_app] Int64 max: {np.iinfo(np.int64).max}")
